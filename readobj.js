...
function readobj(contents) {
    ...
    for (let i = 0; i < indices.length; i+=3) {
        ...
        let face_tangent = vec3.fromValues(
          (t2*x1 - t1*x2)*r,
          (t2*y1 - t1*y2)*r,
          (t2*z1 - t1*z2)*r
        );
        let face_bitangent = -vec3.fromValues(
          (s1*x2 - s2*x1)*r,
          (s1*y2 - s2*y1)*r,
          (s1*z2 - s2*z1)*r
        );
        ...
        for (let j = 0; j < 3; j++) {
            ...
            //} else {
                ...
                vec3.scale(tangent, normal, vec3.dot(normal, face_tangent))
                vec3.subtract(tangent, face_tangent, tangent);
                vec3.normalize(tangent,tangent);
                // calculate handedness
                let bitangent = vec3.fromValues(0, 1, 0);
                vec3.cross(bitangent, normal, tangent);
                let handedness = vec3.dot(bitangent, face_bitangent) < 0 ? -1 : 1;
                // set bitangent
                vec3.cross(bitangent, normal, tangent);
                vec3.scale(bitangent, bitangent, handedness);
